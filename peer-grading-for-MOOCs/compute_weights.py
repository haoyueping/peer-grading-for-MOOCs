                    ...
                    for l5 in range(1,k+1):
                        for l6 in range(1,k+1):
                            ...
                            for j in range(k**2-suml+1):
                                ...
                                c[suml-k+j]=c[suml-k+j]+N_of_sigma(sigma,k)*temp*nCr(k**2-suml,j)*((-1)**j)
...
def find_W(i,j,k,P,perf_tuple,sigma_list,c_list):
    ...
    for s in range(k**2-k):
        for t in range(k**2-k+1):
            weight=weight+(c[s]*d[t])/(s+t+1)*(perf_tuple[1]**(s+t+1)-perf_tuple[0]**(s+t+1))
...
for i in range(sigma_len):
    ...
    curr_time=time.time()
    print ("i = " +str(i)+" time = "+str(curr_time-prev_time)+" seconds")
    prev_time=curr_time
